...
class AWACAgent(DQNAgent):
    ...
    def estimate_advantage(self, ob_no, ac_na, re_n, next_ob_no, terminal_n, n_actions=10):
        ...
        dist = self.awac_actor(ptu.from_numpy(next_ob_no))
        ...
        if self.agent_params['discrete']:
            for i in range(self.agent_params['ac_dim']):
                next_ac_na = np.array([i] * self.batch_size)
                vals.append(self.get_qvals(self.actor.critic, next_ob_no, next_ac_na))
        else:

            for _ in range(n_actions):
                next_ac_na = dist.sample()
                vals.append(self.get_qvals(self.actor.critic, next_ob_no, next_ac_na))

        v_pi = re_n + np.mean(vals, axis=0) * (1 - terminal_n)
    ...
    def step_env(self):
        ...
        trunc = done and 'TimeLimit.truncated' in info and info['TimeLimit.truncated']

        self.last_obs = next_obs.copy()

        if (not self.offline_exploitation) or (self.t <= self.num_exploration_steps):
            self.replay_buffer.store_effect(self.replay_buffer_idx, action, reward, done, trunc)
