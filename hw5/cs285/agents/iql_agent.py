...
class IQLAgent(DQNAgent):
    ...
    def estimate_advantage(self, ob_no, ac_na, re_n, next_ob_no, terminal_n, n_actions=10):
        ...
        v = self.get_qvals(self.exploitation_critic, ob_no, use_v=True)
        q = self.get_qvals(self.exploitation_critic, ob_no, ac_na)
        return (q - v).detach()
        ...
        if (self.t > self.learning_starts
            ...
            if self.normalize_rnd:
                ...
                self.running_rnd_rew_std = (
                                                   1 - self.rnd_gamma) * expl_bonus_std + self.rnd_gamma * self.running_rnd_rew_std
